...
from .wrong_label import wrong_loss
...
class Trainer(object):
    ...
    def __init__(self, info, args):
        ...
        self.params = args
        c= self.params.mul
        self.params.attack_eps = self.params.attack_eps*c
        self.params.attack_step = self.params.attack_step*c
    ...
    def train(self, dataloader, epoch=0, adversarial=False, verbose=True, iadversarial=False, wrong_label=False):
        """
        ...
        for data in tqdm(dataloader, desc='Epoch {}: '.format(epoch), disable=not verbose):
            ...
            if adversarial:
                ...
                loss, batch_metrics = self.iadversarial_loss(x, y)
            elif wrong_label:
                loss, batch_metrics = self.wrong_loss(x, y)#后面尝试加上eps
            else:
    ...
    def wrong_loss(self, x, y):
        loss, batch_metrics = wrong_loss(self.model, x, y, self.optimizer, step_size=self.params.attack_step, 
                                              epsilon=self.params.attack_eps, perturb_steps=self.params.attack_iter, 
                                             attack=self.params.attack, hr_model=self.hr_model)
    ...
    def iadversarial_loss(self, x, y):
        """
        Adversarial training (Madry et al, 2017) with iat examples.
        """
        with ctx_noparamgrad_and_eval(self.model):
            _, r_adv = self.attack.perturb(x, y)
        ##generate iat example as x_iadv here
        x_iadv=x-r_adv
        
        self.optimizer.zero_grad()
        if self.params.keep_clean:
            x_iadv = torch.cat((x, x_iadv), dim=0)
            y_iadv = torch.cat((y, y), dim=0)
        else:
            y_iadv = y
        out = self.model(x_iadv)
        out_clean = self.model(x)
        loss_clean = self.criterion(out_clean, y)
        loss = self.criterion(out, y_iadv)+loss_clean
        
        preds = out.detach()
        batch_metrics = {'loss': loss.item()}
        if self.params.keep_clean:
            preds_clean, preds_iadv = preds[:len(x)], preds[len(x):]
            batch_metrics.update({'clean_acc': accuracy(y, preds_clean), 'inverse_adversarial_acc': accuracy(y, preds_iadv)})
        else:
            batch_metrics.update({'inverse_adversarial_acc': accuracy(y, preds)})    
        return loss, batch_metrics
    ...
    def eval(self, dataloader, adversarial=False, iadversarial=False,wrong_label=False):
        """
        ...
        for x, y in dataloader:
            ...
            if adversarial:
                ...
                with ctx_noparamgrad_and_eval(self.model):
                    _, r_adv = self.eval_attack.perturb(x, y)            
                out = self.model(x-r_adv)
            elif wrong_label:
                with ctx_noparamgrad_and_eval(self.model):
                    x_adv, _ = self.eval_attack.perturb(x, y)            
                out = self.model(x_adv)
    ...
    def save_and_eval_iadversarial(self, dataloader, save, verbose=False, to_true=False, save_all=True):
        """
        Evaluate inverse adversarial accuracy and save perturbations.
        """
        if save_all:
            x_all, y_all = [], []
        r_adv_all = []
        ...
        for x, y in tqdm(dataloader, disable=not verbose):
            ...
            with ctx_noparamgrad_and_eval(self.model):
                if to_true:
                    ...
                    _, r_adv = self.eval_attack.perturb(x)
                    
            out = self.model(x-r_adv)
            acc_iadv += accuracy(y, out)
            if save_all: 
                x_all.append(x.cpu().numpy())
                y_all.extend(y.cpu().numpy())
            r_adv_all.append((r_adv).cpu().numpy())
                
        acc_iadv /= len(dataloader)
        if save:
            r_adv_all = np.vstack(r_adv_all)
            if save_all: 
                x_all = np.vstack(x_all)
    ...
    # def save_and_eval_wrong_adversarial(self, dataloader, save, verbose=False,  save_all=True, hr_model=self.hr_model):
    #     """
    #     Evaluate  adversarial accuracy on wrong label examples and save perturbations.
    #     """
    #     if save_all:
    #         x_all, y_all = [], []
    #     r_adv_all = []
    ...
    #         y_max=hr_model(x_adv)
    #         out = self.model(x_adv)
    #         acc_iadv += accuracy(y_max, out)
    #         if save_all: 
    #             x_all.append(x.cpu().numpy())
    #             y_all.extend(y.cpu().numpy())
    #         r_adv_all.append((r_adv).cpu().numpy())
