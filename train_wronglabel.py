"""
AT using adversarial examples but with wrong label
"""
...
NUM_WRONG_LABEL=args.num_wrong_label
...
for epoch in range(1, NUM_STD_EPOCHS+1):
    ...
    if epoch == NUM_STD_EPOCHS:
        test_adv_acc = trainer.eval(test_dataloader, wrong_label=True)
...
# Wrong_label Adversarial Training (AT, TRADES, MART and HAT)
if NUM_WRONG_LABEL > 0:
    ...
    old_score = [0.0, 0.0]
    logger.log('Wrong label Adversarial training for {} epochs'.format(NUM_WRONG_LABEL))
    trainer.init_optimizer(args.num_wrong_label)
    test_adv_acc = 0.0    
    

for epoch in range(1, NUM_WRONG_LABEL+1):
    start = time.time()
    logger.log('======= Epoch {} ======='.format(epoch))
    ...
    res = trainer.train(train_dataloader, epoch=epoch, wrong_label=True)
    test_acc = trainer.eval(test_dataloader)
    ...
    if epoch % args.adv_eval_freq == 0 or epoch > (NUM_WRONG_LABEL-5) or (epoch >= (NUM_WRONG_LABEL-10) and NUM_WRONG_LABEL > 90):
        test_adv_acc = trainer.eval(test_dataloader, wrong_label=True)
...
# Record metrics
#这里有点小问题，保存的应该是test accuracy最大的
