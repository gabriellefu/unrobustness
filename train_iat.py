...
setting a eps which is the 

Generating examples which has the least loss comparing with original images, and used them as 
IAT examples, then justing using adversarial training with these examples, which is including those
examples into the training data, and the label of these example are the original labels.

output 
1 the accuracy of the trained model on original data
2 the accuracy of the trained model on IAT data
3 the accuracy of the adverial data
"""



'''
5.7: generate iat example from x-r_adv to see it's result
...
NUM_IAT_EPOCHS=args.num_iadv_epochs
...
for epoch in range(1, NUM_STD_EPOCHS+1):
    ...
    epoch_metrics.update({'epoch': epoch, 'lr': last_lr, 'test_clean_acc': test_acc, 'test_inverse_adversarial_acc': ''})
    
    if epoch == NUM_STD_EPOCHS:
        test_iadv_acc = trainer.eval(test_dataloader, iadversarial=True)
        logger.log('Inverse Adversarial Accuracy-\tTest: {:.2f}%.'.format(test_iadv_acc*100))
        epoch_metrics.update({'test_inverse_adversarial_acc': test_iadv_acc})
...
if NUM_IAT_EPOCHS > 0:
    ...
    if args.exp:
        ...
        _ = trainer.save_and_eval_iadversarial(train_eval_dataloader, save=os.path.join(TMP, 'eval_train_iadv_0'))
        _ = trainer.save_and_eval_iadversarial(test_eval_dataloader, save=os.path.join(TMP, 'eval_test_iadv_0'))
    
    old_score = [0.0, 0.0]
    logger.log('Inverse Adversarial training for {} epochs'.format(NUM_IAT_EPOCHS))
    trainer.init_optimizer(args.num_iadv_epochs)
    test_iadv_acc = 0.0    
    

for epoch in range(1, NUM_IAT_EPOCHS+1):
    start = time.time()
    logger.log('======= Epoch {} ======='.format(epoch))
    ...
    res = trainer.train(train_dataloader, epoch=epoch, iadversarial=True)
    test_acc = trainer.eval(test_dataloader)

    if args.exp and (epoch % 5 == 0 or epoch == 1):
        trainer.save_model(os.path.join(TMP, 'model_{}.pt'.format(epoch)))
        save_eval_train_file = os.path.join(TMP, 'eval_train_iadv_{}'.format(epoch))
        save_eval_test_file = os.path.join(TMP, 'eval_test_iadv_{}'.format(epoch))
        _ = trainer.save_and_eval_iadversarial(train_eval_dataloader, save=save_eval_train_file, save_all=False)
        _ = trainer.save_and_eval_iadversarial(test_eval_dataloader, save=save_eval_test_file, save_all=False)
    ...
    epoch_metrics = {'train_'+k: v for k, v in res.items()}
    epoch_metrics.update({'epoch': NUM_STD_EPOCHS+epoch, 'lr': last_lr, 'test_clean_acc': test_acc, 'test_inverse_adversarial_acc': ''})
    
    if epoch % args.adv_eval_freq == 0 or epoch > (NUM_IAT_EPOCHS-5) or (epoch >= (NUM_IAT_EPOCHS-10) and NUM_IAT_EPOCHS > 90):
        test_iadv_acc = trainer.eval(test_dataloader, iadversarial=True)
        logger.log('Inverse Adversarial Accuracy-\tTrain: {:.2f}%.\tTest: {:.2f}%.'.format(res['inverse_adversarial_acc']*100, 
                                                                                   test_iadv_acc*100))
        epoch_metrics.update({'test_inverse_adversarial_acc': test_iadv_acc})
    else:
        logger.log('Inverse Adversarial Accuracy-\tTrain: {:.2f}%.'.format(res['inverse_adversarial_acc']*100))
    
    if test_acc >= old_score[0]:
        old_score[0], old_score[1] = test_acc, test_iadv_acc
        trainer.save_model(WEIGHTS)
    trainer.save_model(os.path.join(LOG_DIR, 'weights-last.pt'))

    logger.log('Time taken: {}'.format(format_time(time.time()-start)))
    metrics = metrics.append(pd.DataFrame(epoch_metrics, index=[0]), ignore_index=True)
    metrics.to_csv(os.path.join(LOG_DIR, 'stats_iadv.csv'), index=False)
...
if NUM_IAT_EPOCHS > 0:
    logger.log('Inverse Adversarial Accuracy-\tTrain: {:.2f}%.\tTest: {:.2f}%.'.format(res['inverse_adversarial_acc']*100, old_score[1]*100)) 
